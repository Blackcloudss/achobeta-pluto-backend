...
type Team_Member_Structure struct {
	CommonModel
	TeamId      int64 `gorm:"column:team_id; type:bigint; index:idx_teamid_memberid_structid; not null; comment:'团队ID'"`
	MemberId    int64 `gorm:"column:member_id; type:bigint; index:idx_teamid_memberid_structid; not null; comment:'成员ID'"`
	StructureId int64 `gorm:"column:structure_id; type:bigint; index:idx_teamid_memberid_structid; not null; comment:'职位ID'"`
...
type User_Power struct {
	CommonModel
	MemberId int64 `gorm:"column:member_id; type:bigint;   index:idx_member_team_level;not null; comment:'成员ID'"`
	TeamId   int64 `gorm:"column:team_id;   type:bigint;   index:idx_member_team_level;not null; comment:'团队ID'"`
	Level    int   `gorm:"column:level;     type:int;      index:idx_member_team_level;not null; default:1; comment:'权限等级'"`
